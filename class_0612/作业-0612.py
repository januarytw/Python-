...
class ReadData():
    ...
    def ReadExcelToList(self,file,sheetName):#此处可以改进一下，利用初始化函数
        try:
            work_book=load_workbook(file)
            sheet=work_book[sheetName]##工作表的名字可以作为参数
            row=sheet.max_row
...
result=ReadData().ReadExcelToList("case.xlsx","test_data")
print(result[0][2],result[3][0])
